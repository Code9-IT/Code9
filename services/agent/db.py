...
async def init_pool(retries: int = 15, delay: float = 3.0):
    """
    Create the connection pool.  Retries so the agent can start
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"[agent] DB pool ready (attempt {attempt})")
            return
        except Exception as exc:                    # pragma: no cover
            print(f"[agent] DB not ready – attempt {attempt}/{retries}: {exc}")
